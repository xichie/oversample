...
import logging
from util import sampling, evaluate, MMD
...
class GAN():
    ...
    def train(self, pos, neg, epochs=500, batch_size=32):
        ...
        neg_label[:, 0] = 1.
        pos_label = np.zeros(shape=(batch_size, 3))
        pos_label[:, 1] = 1.
        ...
        for epoch in range(epochs):
            # ---------------------
            #  Train Discriminator and M
            # ---------------------
            idx = np.random.randint(0, neg.shape[0], batch_size)
            idx1 = np.random.randint(0, pos.shape[0], batch_size)
            noise = np.random.normal(0, 1, (batch_size, self.latent_dim))
            gen_data = self.generator.predict(noise)

            
            d_loss_neg = self.M.train_on_batch(neg[idx], neg_label)  # [1, 0, 0]
            d_loss_pos = self.M.train_on_batch(pos[idx1], pos_label)  # [0, 1, 0]
            d_loss_gen = self.M.train_on_batch(gen_data, gen_label)  # [0, 0, 1]
            d_loss = 0.5 * np.add(d_loss_gen, 0.5 * np.add(d_loss_neg, d_loss_pos))
			
            self.discriminator1.train_on_batch(pos[idx1], real)
            self.discriminator1.train_on_batch(gen_data, fake)

            self.discriminator2.train_on_batch(pos[idx1], fake)
            self.discriminator2.train_on_batch(gen_data, real)
            # ---------------------
            #  Train Generator
            # ---------------------
            ...
            g_loss = self.combined1.train_on_batch(noise, real)
            g_loss = self.combined2.train_on_batch(noise, fake)
            # Plot the progress
...
if __name__ == '__main__':
    ...
    for (x_train, y_train), (x_test, y_test) in zip(train, test):
        ...
        gan = GAN()
        X_sample, y_sample = sampling(gan, x_train_std, y_train, oversample_num=1000, epochs=1000, iter=5)
